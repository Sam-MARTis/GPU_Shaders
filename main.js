...
void main() {
    // vPosition = vec3(smoothstep(0.3, 0.7, (0.5+0.5*sin((normal.xxx)*30.0))));
    // vPosition = vec3(normal);
    vPosition = position;
    vec3 coords = normal;
    coords.y += uTime/1000.0;
    vec3 noisePattern = vec3(noise(coords));
    vUv = uv;

    // vec3(smoothstep(0.3, 0.7, (0.5+0.5*sin((vNorTrans.xxx)*30.0))));
    vec3 pattern =vec3((0.5+0.5*sin((noisePattern)*30.0)));

    vDisplacement = pattern;

	gl_Position = projectionMatrix * viewMatrix*modelMatrix * vec4( vPosition + (normal*vDisplacement/3.0) , 1.0 );
}

`

const makeRender = (t=0) => {
    material.uniforms.uTime.value = t/2.0;
...
const geo = new THREE.IcosahedronGeometry(1.0, 200);
